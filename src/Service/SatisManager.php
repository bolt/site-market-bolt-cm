...
class SatisManager
{
    ...
    {
        $queuePendingDir = $this->resourceManager->getPath('cache/.satis/webhook/pending');
        $queueProcessedDir = $this->resourceManager->getPath('cache/.satis/webhook/processed');
     ...
     */
    public function processQueue(PackageManager $packageManager, OutputInterface $output = null)
    {
        if ($output === null) {
            $output = new NullIO();
        }

        $this->processQueuedWebhooks($packageManager, $output);
        $this->processQueuedPackages($output);
    }
     ...
     */
    protected function processQueuedWebhooks(PackageManager $packageManager, OutputInterface $output)
    {
        $lockDir = $this->resourceManager->getPath('cache/.satis/lock');
        $queuePendingDir = $this->resourceManager->getPath('cache/.satis/webhook/pending');
        $queueProcessedDir = $this->resourceManager->getPath('cache/.satis/webhook/processed');
     ...
     * @param OutputInterface $output
     */
    protected function processQueuedPackages(OutputInterface $output)
    {
        $lockDir = $this->resourceManager->getPath('cache/.satis/lock');
        $queueDir = $this->resourceManager->getPath('cache/.satis/queue');
